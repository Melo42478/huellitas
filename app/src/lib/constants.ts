...
export function whatsappAdoptLink(dogName: string, customWhatsapp?: string | null): string {
  const whatsappNumber = customWhatsapp ? `https://wa.me/52${customWhatsapp}` : WHATSAPP_BASE;
  return `${whatsappNumber}?text=${encodeURIComponent(`Hola, me interesa adoptar a ${dogName} 🐾`)}`;
}
